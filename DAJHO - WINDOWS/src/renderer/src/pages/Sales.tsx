...
// Carga diferida del PDF (reduce el bundle inicial en ~1.5MB)
const ReciboPDFDownload = lazy(() => 
  Promise.all([
    import('../components/ReciboPDF'),
    import('@react-pdf/renderer')
  ]).then(([reciboModule, pdfModule]) => ({
    default: ({ data }: { data: any }) => {
      const { ReciboPDF } = reciboModule;
      const { PDFDownloadLink } = pdfModule;
      return (
        <PDFDownloadLink
          document={<ReciboPDF data={data} />}
          fileName={`Recibo_${data.numero || data.id}.pdf`}
        ...
        >
          {({ loading }: { loading: boolean }) => (
            <div style={{
              ...
              backgroundColor: '#059669', color: '#fff', border: 'none', fontSize: 14, fontWeight: 600,
            }}>
              <FileText size={18} />
        ...
        <div style={s.modalOverlay}>
          <div style={s.modal}>
            ...
            <div style={{ ...s.modalActions, flexDirection: 'column', gap: 10 }}>
              <Suspense fallback={
                <div style={{ ...s.modalConfirm, textAlign: 'center' }}>
                  <Loader size={18} style={{ marginRight: 6 }} /> Cargando PDF...
                </div>
              }>
                <ReciboPDFDownload data={{
